...
const { 
  ...
  const handleMultiImageChange = (e) => {
    ...
    if (validFiles.length !== files.length) {
      setImageError("Some files are invalid. Only JPG, PNG, WebP files under 5MB are allowed.");
      return;
    }
  ...
  const getMultiImageRootProps = () => ({
    onClick: () => multiImageInputRef.current?.click(),
  });

  const getMultiImageInputProps = () => ({
  ...
  const handleAiImageChange = (e) => {
    ...
    if (!isValidType || !isValidSize) {
      setImageError("Invalid file. Only JPG, PNG, WebP files under 5MB are allowed.");
      return;
    }
  ...
  const getAiRootProps = () => ({
    onClick: () => aiImageInputRef.current?.click(),
  });

  const getAiInputProps = () => ({
    ref: aiImageInputRef,
    type: "file",
    accept: "image/jpeg,image/jpg,image/png,image/webp",
  ...
  const processWithAI = async () => {
    ...
    // TODO: Implement AI processing with Gemini API
    // For now, just simulate loading
    setTimeout(() => {
      setProcessImageLoading(false);
      // You can add logic here to process the image and populate form fields
    }, 2000);
  };
